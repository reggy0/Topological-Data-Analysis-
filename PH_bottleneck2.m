function d = PH_bottleneck2(C1,C2)
%function d = PH_bottleneck2(C1,C2)
%
% Bottleneck distance for the persistence diagrams of connected components 
% obtained by a graph filtration. Assumes the equal number of points in 
% persistent diagram and does not require argumenting data long the line y=x.
%
% INPUT
% connectivity (distance) matrix C1, C2 of size p x p
%
%
% OUTPUT
% d: bottleneck distance between the persistence diagrams of connected
% components of networks. This code uses the function 'mukres' coded by 
% Yi Cao at Cranfield University.  
%
% The mathematical details of the code is given in the following paper
% Chung, M.K. Lee, H., Gritsenko, A., DiChristofano, A., Pluta, D. 
% Ombao, H. Solo, V. Topological Brain Network Distances, ArXiv 1809.03878
% http://arxiv.org/abs/1809.03878
%
% If you are using this code, please reference tha above paper. 
%
%
% (C) 2018 Hyekyung Lee, Seoul National University
%          Moo K. chung, University of Wisconsin-Madison
% 2018 August 11



% Estimate the death of connected components 
% The birth of connected components is always zero. 
% For the network C1,
[Tree,Pred] = graphminspantree(sparse(C1)); 
val = nonzeros(Tree); 
X = [zeros(length(val),1) sort(val,'ascend')]; 
% For the network C1,
[Tree,Pred] = graphminspantree(sparse(C2)); 
val = nonzeros(Tree); 
Y = [zeros(length(val),1) sort(val,'ascend')]; 
...
for i = 1:p,
    for j = 1:q,
        dist_xtoy(i,j) = max(abs(X(i,:) - Y(j,:)));
    end
end
dist = dist_xtoy; 

% find the correspondence from X to Y that minimizes the sum of distances 
[assignment,cost] = munkres(dist);
tmp = dist((assignment-1)*(p)+[1:length(assignment)]);
d = max(tmp); 
