% This code generates the validation table 1 for expected topological loss 
% published in Das, S., Anand, D.V., Chung, M.K. 2022 Topological data analysis 
% for human brain networks through order statistics arXiv:2204.02527.
%
...
for iter=1:nrepeats
    ...
    % Observed statistics
    observed_distance = sum((mean(inv_F_b_x)-mean(inv_F_b_y)).^2) + sum((mean(inv_F_d_x)-mean(inv_F_d_y)).^2); 
    ...
    for i=1:per_s %each iteration gives a permutation
        ...
        stat_s(i,iter) = sum((mean(xper_b)-mean(yper_b)).^2) + sum((mean(xper_d)-mean(yper_d)).^2);
     end
end
toc
...
set(gca, 'fontsize',16)
%print(gcf,'etl_hist2.png','-dpng','-r300'); 
